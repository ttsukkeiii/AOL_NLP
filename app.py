...
/* MCQ card */
.mcq-card {
    background: linear-gradient(145deg, #1e2f3f, #162333);
...
.option-label.correct { color: #4fc3f7; }
...
def form_question(sentence, q_type, answer):
    ...
    templates = {
        ...
        "where":        f"Where did the events described take place?",
        ...
        "what percent": f"What percentage is described in this passage?",
...
with st.sidebar:
    ...
    if models_loaded:
        ...
        st.markdown("""
        **Letakkan file berikut di folder yang sama dengan `app.py`:**
        - `rf_model.pkl`
        - `feature_cols.pkl`

        Kedua file ini dihasilkan dari notebook Colab (Section 10).
        """)

    st.markdown("---")
    st.markdown("### 👥 Team")
    st.markdown("""
    <p>Catherine Sharon<br>Cherysh Amanda Ovelia<br>Daryl Rafael Wiguna<br>Kezia Gracella Haryono<br>Rebecca Jielian</p>
    """, unsafe_allow_html=True)

# ─── UI: Main ─────────────────────────────────────────────────────────────────
if not models_loaded:
    st.warning(
        "⚠️ `rf_model.pkl` dan `feature_cols.pkl` tidak ditemukan. "
        "Pastikan kedua file tersebut ada di folder yang sama dengan `app.py`. "
        "File ini dihasilkan dari notebook Colab (Section 10 — Save Models).",
        icon="⚠️"
    )
    st.stop()
...
if "example_text" in st.session_state and not input_text:
...
if generate_btn:
    if not input_text.strip():
    ...
    elif len(get_sentences(input_text)) < 3:
        ...
        with st.spinner("🔍 Analysing text with TextRank + Random Forest…"):
            mcqs = generate_questions(input_text, n_questions=n_questions)
        ...
        if not mcqs:
            ...
            st.markdown('<div class="section-title">📝 Generated Questions</div>',
                        unsafe_allow_html=True)
            ...
            for i, mcq in enumerate(mcqs, 1):
                ...
                for lbl, opt in mcq["options"].items():
                    ...
                    css_class  = "option-row correct" if is_correct else "option-row"
                    lbl_class  = "option-label correct" if is_correct else "option-label"
                    check      = '<span class="check-icon">✓</span>' if is_correct else ""
                    options_html += f"""
                    <div class="{css_class}">
                        <span class="{lbl_class}">{lbl}.</span>
                        <span>{opt}</span>
                        {check}
                    </div>"""
                ...
                answer_row = ""
                if show_answer:
                    answer_row = f'<div class="answer-key">🔑 Answer: <strong>{mcq["answer_key"]}. {mcq["answer_text"]}</strong></div>'
                ...
                <div class="mcq-card">
                    <div>
                        <span class="mcq-number">Question {i}</span>
                        <span class="mcq-type-badge">{mcq["type"]}</span>
                    </div>
